...
<%--
	prefix : 접두사
	uri(Uniform Resurce Indentifier) : 자원을 구분하는 식별자(주소형태)
...
표기법을 간단히 태그화 하여 표준으로 제공하는것.

(if,for 배열.컬렉션 길이, 문자열 치환 숫자 데이터 형식 변경
데이터 파싱의 scope 변수 선언 등)
...
<li>webapp/WEB-INF/lib폴더 에 추가하기</li>
</ol>
...
<pre>
JSP파일 최상단에 추가하고자 하는 JSPL taglib를 추가
</pre>
   ...
   <%--request scope에 num2세팅 --%>
    ...
    <h4>${num2 }</h4> <!-- 20 -->
     <h4>${requestScope.num2 }</h4> <!-- 300 -->
       ...
       <h4>temp ${temp }</h4><!-- 임시 값 아닌가? -->
       
       <hr>
       
       <h1> 2. 변수 삭제(c:remove)</h1>
       <pre>
       - c:set / setAttribute() 로 추가된 값
       
       -c:remove 속성
       1) var : 삭제할 변수명(key)
       2)scope: 삭제할 내장 객체 범위(기본값 : 모든범위)
     ...
     <h4>sessionscope.test ==sessione</h4>
     </c:if>
