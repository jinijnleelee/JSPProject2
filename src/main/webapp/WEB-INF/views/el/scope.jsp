...
<head>
    ...
    <title>2. Servlet/JSP내장 객체와 범위(scope)</title>
</head>
<body>
<h1>Servlet/JSP내장 객체와 범위(scope)</h1>
<pre>
		Servlet/JSP에는 기본적으로 내장되어있는 4가지 객체가 존재함.
		
		4종류의 객체는 각각 영향을 미칠수있는 범위가 다름
		
		1. page: 현재 페이지
		-> 현재 Servlet또는 JSP에서만 사용가능(1페이지)
		...
		(세션에 로그인 정보를 기록해둠)
		-> 브라우저가 종료되거나 로그아웃 되기전까지
		계속 로그인 상태 유지됨.)
		
		4. apprication : 하나의 웹 애플리케이션 당 1개만 생성되는 객체.
		-> 서버 시작시 생성되며 종료시 까지 유지 
		-> 누구든지 사용가능 
		
		/****내장 객체의 우선 순위****/
		
		-> setAttribute("value")로 내장 객체가 값을 세팅할때
		key값이 중복되는 경우
		
		/${key }로 작성하는 경우
		범위가 작은 내장 객체가 높은 우선순위를 갖게 된다
		page> reqyest > session > application
